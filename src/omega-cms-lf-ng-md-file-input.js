(function(window,angular,undefined) {
    ...
    var omegaCmsLfNgMdFileInput = angular.module('omegaCmsLfNgMdFileInput', ['ngMaterial']);

    omegaCmsLfNgMdFileInput.directive('lfFile', function() {
        return {
            restrict: 'E',
            scope: {
                lfFileObj: '=',
                    ...
                    default : {
                        ...
                        element.replaceWith(
                            '<object type="' + fileType + '" data="' + src + '">' +
                                '<div class="omega-cms-lf-ng-md-file-input-preview-default">' +
                                    '<md-icon class="omega-cms-lf-ng-md-file-input-preview-icon '+unKnowClass+'"></md-icon>' +
                                '</div>' +
                            '</object>'
                        );
                    }
    ...
    omegaCmsLfNgMdFileInput.run(['$templateCache', function($templateCache){
        $templateCache.put('omegaCmsLfNgMdFileInput.html', [
            '<div layout="column" class="omega-cms-lf-ng-md-file-input" ng-model="'+genLfObjId()+'">',
                '<div layout="column" class="omega-cms-lf-ng-md-file-input-preview-container" ng-class="{\'disabled\':isDisabled}" ng-show="isDrag || (isPreview && lfFiles.length)">',
                    '<md-button aria-label="remove all files" class="close omega-cms-lf-ng-md-file-input-x" ng-click="removeAllFiles($event)" ng-hide="!lfFiles.length || !isPreview" >&times;</md-button>',
                    '<div class="omega-cms-lf-ng-md-file-input-drag">',
                        '<div layout="row" layout-align="center center" class="omega-cms-lf-ng-md-file-input-drag-text-container" ng-show="(!lfFiles.length || !isPreview) && isDrag">',
                            '<div class="omega-cms-lf-ng-md-file-input-drag-text">{{strCaptionDragAndDrop}}</div>',
                        '</div>',
                        '<div class="omega-cms-lf-ng-md-file-input-thumbnails" ng-if="isPreview == true">',
                            '<div class="omega-cms-lf-ng-md-file-input-frame" ng-repeat="lffile in lfFiles" ng-click="onFileClick(lffile)">',
                                '<div class="omega-cms-lf-ng-md-file-input-x" aria-label="remove {{lffile.lFfileName}}" ng-click="removeFile(lffile,$event)">&times;</div>',
                                '<lf-file lf-file-obj="lffile" lf-unknow-class="strUnknowIconCls"/>',
                                // '<md-progress-linear md-mode="indeterminate"></md-progress-linear>',
                                '<div class="omega-cms-lf-ng-md-file-input-frame-footer">',
                                    '<div class="omega-cms-lf-ng-md-file-input-frame-caption">{{lffile.lfFileName}}</div>',
                                '</div>',
                            '</div>',
                        '</div>',
                    ...
                    '</div>',
                '</div>',
                '<div layout="row" class="omega-cms-lf-ng-md-file-input-container" >',
                    '<div class="omega-cms-lf-ng-md-file-input-caption" layout="row" layout-align="start center" flex ng-class="{\'disabled\':isDisabled}" >',
                        '<md-icon class="lf-icon" ng-class="strCaptionIconCls"></md-icon>',
                        '<div flex class="omega-cms-lf-ng-md-file-input-caption-text-default" ng-show="!lfFiles.length">',
                            '{{strCaptionPlaceholder}}',
                        '</div>',
                        '<div flex class="omega-cms-lf-ng-md-file-input-caption-text" ng-hide="!lfFiles.length">',
                    ...
                    '</div>',
                    '<md-button aria-label="remove all files" ng-disabled="isDisabled" ng-click="removeAllFiles()" ng-hide="!lfFiles.length || intLoading" class="md-raised omega-cms-lf-ng-md-file-input-button omega-cms-lf-ng-md-file-input-button-remove" ng-class="strRemoveButtonCls">',
                        '<md-icon class="lf-icon" ng-class="strRemoveIconCls"></md-icon> ',
                        '{{strCaptionRemove}}',
                    '</md-button>',
					'<md-button aria-label="submit" ng-disabled="isDisabled" ng-click="onSubmitClick()" class="md-raised md-warn omega-cms-lf-ng-md-file-input-button omega-cms-lf-ng-md-file-input-button-submit" ng-class="strSubmitButtonCls" ng-show="lfFiles.length && !intLoading && isSubmit">',
    ...
    omegaCmsLfNgMdFileInput.filter('lfTrusted', ['$sce', function ($sce) {
        return function(url) {
            return $sce.trustAsResourceUrl(url);
        };
    }]);

    omegaCmsLfNgMdFileInput.directive('lfRequired', function() {
        return {
            restrict: "A",
            require:"ngModel",
    ...
    omegaCmsLfNgMdFileInput.directive('lfMaxcount', function() {
        return {
            restrict: "A",
            require:"ngModel",
            link: function(scope, element, attrs ,ctrl) {
    ...
    omegaCmsLfNgMdFileInput.directive('lfFilesize', function() {
        return {
            restrict: "A",
            require:"ngModel",
            link: function(scope, element, attrs ,ctrl) {
	...
	omegaCmsLfNgMdFileInput.directive('lfTotalsize', function() {
        return {
            restrict: "A",
            require:"ngModel",
            link: function(scope, element, attrs ,ctrl) {
    ...
    omegaCmsLfNgMdFileInput.directive('lfMimetype', function() {
        return {
            restrict: "A",
            require:"ngModel",
            link: function(scope, element, attrs ,ctrl) {
    ...
    omegaCmsLfNgMdFileInput.directive('omegaCmsLfNgMdFileInput',['$q','$compile','$timeout', function($q,$compile,$timeout){
        return {
            restrict: 'E',
            templateUrl: 'omegaCmsLfNgMdFileInput.html',
            replace: true,
            require:"ngModel",
            scope:{
                lfFiles:'=?',
            ...
            link: function(scope,element,attrs,ctrl){
                ...
                var elFileinput = angular.element(element[0].querySelector('.omega-cms-lf-ng-md-file-input-tag'));
                var elDragview  = angular.element(element[0].querySelector('.omega-cms-lf-ng-md-file-input-drag'));
                var elThumbnails = angular.element(element[0].querySelector('.omega-cms-lf-ng-md-file-input-thumbnails'));
                var intFilesCount = 0;
                ...
                elDragview.bind("dragover", function(e){
                    ...
                    elDragview.addClass("omega-cms-lf-ng-md-file-input-drag-hover");
                });
				...
				elDragview.bind("dragleave", function(e){
					...
					elDragview.removeClass("omega-cms-lf-ng-md-file-input-drag-hover");
				});
                ...
                var onFileEvent = function(eventName) {
					if(angular.isFunction(scope.ngFileEvent)){
						scope.ngFileEvent({ event: eventName, files: scope.lfFiles });
					}
                }
